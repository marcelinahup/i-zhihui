...
function RegionSelect(selRegionProp) {
	...
	_allRegion = _regions;
	_selectedClass = selRegionProp["selectedClass"];
	if (_regions && _regions.length > 0) {
		var _self = this;

		// 选择区域
		_container = selRegionProp.container;
...
RegionSelect.prototype.onBeforeSelect = function(evt) {
	...
	this.isSelect = true;
	//   console.log('down'+_selectedRegions);
}
...
RegionSelect.prototype.onSelect = function(evt) {
	...
	//	console.log('move'+_selectedRegions);
}
...
function showSelDiv() {
	...
	for (var i = 0; i < _allRegion.length; i++) {
		var r = _allRegion[i];
		if (r.className.indexOf(_selectedClass) != -1) {
			var userIdVal = $(r).find('.userId').val();
			var userId = new Object();
			userId.userId = userIdVal;
			selInfo.push(userId);
		}
	}
	//   for(var i=0; i<arr.length;i++){
	//	   var userIdVal=$(arr[i]).find('.userId').val();
	//	   var userId=new Object();
	//	   userId.userId=userIdVal;
	//       selInfo.push(userId);
	//   }
